...
async def send_home(msg_or_call, user_id: int, state: FSMContext = None):
    """إرسال / تحديث شاشة البانر الرئيسية."""
...
async def bm_ad_id_input(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    await reply_ui(bot, state, message.chat.id, "⏳ <b>جارٍ جلب البطاقات من BM...</b>")
...
async def admin_login(message: Message, state: FSMContext):
    ...
    await message.answer("🔐 <b>لوحة تحكم خاصة</b>\n\nأدخل كلمة المرور:")
...
async def admin_password(message: Message, state: FSMContext):
    if not _is_admin_user(message.from_user.id):
        await state.clear()
        await message.answer("⛔ غير مصرح لك باستخدام لوحة الأدمن.")
        return
    if message.text.strip() != ADMIN_PASS:
        await message.answer("❌ كلمة المرور خاطئة.")
        return
    await state.set_state(AdminFlow.authenticated)
    c = db.counts()
    await message.answer(
        f"✅ <b>مرحباً بك في لوحة التحكم</b>\n\n"
        f"📊 {c['users']} مشترك نشط | 📈 {c['requests']} طلب",
...
@dp.message(AdminFlow.waiting_extend_duration)
async def admin_extend_duration(message: Message, state: FSMContext):
    data = await state.get_data()
    ...
    try:
        ...
        return
    row   = db.get_user(uid)
    db.add_user(uid, row['username'] if row else '', row['first_name'] if row else '')
    until = db.set_subscription_hours(uid, hours)
    await state.clear()
    days   = hours // 24
    period = f"{days} يوم" if days >= 1 else f"{hours} ساعة"
    await message.answer(
